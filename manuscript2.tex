...
Our secondary contribution is the application of this framework to two major bibliographic infrastructures for Library and Information Science---Dimensions.ai and OpenAlex---and its extension across nine additional disciplinary citation graphs covering the natural sciences, engineering, biomedicine, social sciences, and humanities. The LIS datasets are treated as complementary experiments rather than as a perfectly controlled comparison because their metadata exposures differ fundamentally: Dimensions provides a structural-only baseline derived primarily from citation topology and metadata, whereas OpenAlex enables the construction of a semantic+structural framework through abstract availability. The ten-discipline comparative study allows us to investigate whether the structural and semantic mechanisms of citation behavior are universal properties of scholarly communication or vary systematically by disciplinary culture.
...
All ten datasets were constructed from the Dimensions database \citep{hook2018dimensions}, a comprehensive, linked research information system integrating publications, citations, grants, patents, and policy documents. Its coverage is among the broadest available, and it has been shown to provide a high degree of completeness and quality in publication metadata \citep{delgado2024completeness, nguyen2022quality}. Programmatic access to the Dimensions API was facilitated via the \texttt{dimcli} Python client \citep{dimcli2021}. For each dataset, we queried the Dimensions DSL using a combination of Field of Research codes and keyword filters targeting the title and abstract, covering the period 1975--2024.

For the Library and Information Science (LIS) dataset, we additionally constructed a complementary OpenAlex \citep{priem2022openalex} dataset to enable a semantic+structural framework through abstract availability. OpenAlex is a fully open, freely accessible index of scholarly works developed as a successor to Microsoft Academic Graph. It has been independently assessed as providing reliable coverage and metadata quality for bibliometric research \citep{delgado2024completeness, nguyen2022quality}, and its open data philosophy makes it particularly suitable for reproducible research. The Dimensions LIS dataset and the OpenAlex LIS dataset are treated as complementary experiments rather than as a perfectly controlled comparison because their metadata availability differs fundamentally: Dimensions provides a structural-only baseline, whereas OpenAlex enables a semantic+structural framework through abstract availability. The use of two independent data infrastructures for LIS strengthens the robustness of our findings: results consistent across Dimensions and OpenAlex are more likely to reflect genuine properties of LIS citation dynamics rather than artifacts of a specific data source.

\subsection{The Ten Citation Graphs}

The ten datasets span five broad disciplinary clusters: natural sciences (Dark Matter, LIS/Information Literacy), engineering (Fatigue Crack, Environmental Engineering), biomedicine (Neuroblastoma, Osteosarcoma), social sciences (Political Participation, Welfare State), and humanities (Archaeology, Art History). Table~\ref{tab:datasets} summarizes the topological characteristics of the ten citation graphs.

\begin{table}[H]
\centering
\caption{Descriptive statistics of the ten disciplinary citation graphs extracted from Dimensions (1975--2024).}
\label{tab:datasets}
\resizebox{\textwidth}{!}{%
...
\begin{enumerate}
    ...
    Text embeddings derived from pretrained language models may implicitly encode citation contexts if the target papers were included in the pretraining corpus. We employ Sentence-BERT (\texttt{all-MiniLM-L6-v2}) to compute semantic similarity. We acknowledge that the static pre-training corpus of SBERT introduces a systemic vector of semantic leakage that cannot be mitigated without chronologically re-trained language models. A fully leakage-free semantic representation would require rolling vocabulary fitting or temporally constrained re-training, which we identify as future work.
\end{enumerate}

\subsection{Bibliographic Data Collection}

\textbf{Dimensions Datasets.} The Dimensions database \citep{hook2018dimensions} was queried for each of the ten topics using the Dimensions DSL. For LIS, the query used Field of Research code 4610 and core LIS keywords:
...
After removing records with missing publication years or reference lists, the final LIS Dimensions dataset contained 259,220 deduplicated articles. Analogous queries were constructed for the remaining nine topics using their respective keywords and Field of Research codes. The ten datasets collectively span 1975--2024.

\textbf{OpenAlex Dataset (LIS only).} OpenAlex \citep{priem2022openalex} was queried using five primary topic identifiers corresponding to core LIS domains:
...
The SBERT cosine similarity between the concatenated title and abstract of \(A\) and \(B\), computed using the \texttt{all-MiniLM-L6-v2} model. This representation captures deep semantic alignment between the intellectual content of the two papers. For the structural-only Dimensions LIS baseline, a TF-IDF cosine similarity is used instead, as described in Section~\ref{sec:comp}.
...
To assess temporal robustness, we additionally evaluated model performance using a strict temporal hold-out split (training on papers published $\le 2015$, testing on papers published $2016-2020$). For the LIS datasets, we additionally evaluated performance across four rolling temporal windows. For ranking evaluation (LIS only), we constructed top-\(k\) candidate pools with
\[
k \in \{10,50,100\},
\]
...
We first evaluated Logistic Regression, Linear SVM, Random Forest, and Gradient Boosting on the two LIS datasets using 5-fold stratified cross-validation. Table~\ref{tab:cv} summarizes the predictive performance.

\begin{table}[H]
\centering
\caption{Cross-validation performance (mean $\pm$ standard deviation across 5 folds) for the LIS datasets.}
\label{tab:cv}
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{ROC-AUC} & \textbf{PR-AUC} & \textbf{F1 Score} & \textbf{Brier Score} \\
\midrule
\multicolumn{5}{c}{\textit{Dimensions LIS (Structural-Only Baseline)}} \\
...
\midrule
\multicolumn{5}{c}{\textit{OpenAlex LIS (Semantic+Structural)}} \\
...
\caption{Cross-validation ROC curves for Dimensions LIS (left) and OpenAlex LIS (right). Gradient Boosting achieves the highest ROC-AUC in both datasets.}
\label{fig:cv}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Gradient Boosting cross-validation AUC across 10 datasets (mean $\pm$ std, 5-fold).}
\label{tab:cv_all}
\begin{tabular}{llcc}
\toprule
\textbf{Discipline} & \textbf{Dataset} & \textbf{GB AUC (CV)} & \textbf{GB AUC (Holdout)} \\
\midrule
Science & Dark Matter & 0.877$\pm$0.047 & 0.893 \\
Science & Info Literacy (LIS) & 0.928$\pm$0.014 & 0.918 \\
Engineering & Fatigue Crack & 0.860$\pm$0.013 & 0.896 \\
Engineering & Env.\ Engineering & 0.975$\pm$0.037 & 1.000 \\
BioMed & Neuroblastoma & 0.934$\pm$0.018 & 0.906 \\
BioMed & Osteosarcoma & 0.927$\pm$0.027 & 0.876 \\
Social Science & Political Partic. & 0.931$\pm$0.070 & 0.964 \\
Social Science & Welfare State & 0.953$\pm$0.019 & 0.976 \\
Humanities & Archaeology & 0.874$\pm$0.021 & 0.880 \\
Humanities & Art History & 0.929$\pm$0.021 & 0.925 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Citation Link Prediction AUC (Gradient Boosting, 5-fold CV) across 10 datasets, color-coded by discipline.}
\label{fig:auc}
\end{figure}
...
To evaluate temporal robustness for the LIS datasets, we assessed the Gradient Boosting classifier across four rolling temporal windows using temporally ordered train--test splits (\(t \le Y_{\mathrm{split}}\) for training and \(Y_{\mathrm{split}}+1\) to \(Y_{\mathrm{end}}\) for testing).

\begin{table}[H]
\centering
\caption{Rolling temporal-window ROC-AUC for Gradient Boosting (LIS datasets).}
\label{tab:rolling}
\begin{tabular}{lcc}
\toprule
\textbf{Test Window} & \textbf{Dimensions LIS} & \textbf{OpenAlex LIS} \\
\midrule
...
\caption{Rolling temporal-window ROC-AUC for Dimensions LIS (left) and OpenAlex LIS (right). Performance remains stable across all four temporal windows.}
\label{fig:rolling}
\end{figure}
...
To identify the principal drivers of predictive performance, we performed leave-one-out feature ablation on the Gradient Boosting models for all ten datasets. Table~\ref{tab:ablation_lis} presents the ablation results for the two LIS datasets; Figure~\ref{fig:ablation} and Figure~\ref{fig:shap} present the ten-discipline ablation and SHAP heatmaps.

\begin{table}[H]
\centering
\caption{Feature ablation measured as ROC-AUC decrease after feature removal (LIS datasets).}
\label{tab:ablation_lis}
\begin{tabular}{lcc}
\toprule
\textbf{Feature} & \textbf{Dimensions LIS Drop} & \textbf{OpenAlex LIS Drop} \\
\midrule
...
\caption{Feature ablation for Dimensions LIS (left) and OpenAlex LIS (right). Semantic similarity dominates predictive performance in OpenAlex, whereas temporal gap and co-citation overlap are the strongest structural features in Dimensions.}
...
\caption{SHAP feature importance for Dimensions LIS (left) and OpenAlex LIS (right). Consistent with the ablation analysis, semantic similarity is the dominant contributor in OpenAlex.}
\label{fig:shap_lis}
\end{figure}
...
Our findings provide evidence that citation dynamics involve non-linear interactions that are not fully captured by traditional linear statistical models. The performance gap between Gradient Boosting and Logistic Regression (approximately 0.023 on Dimensions LIS and 0.008 on OpenAlex LIS), while incremental in absolute terms, is consistent across cross-validation folds and temporal windows. This gap is compatible with the presence of non-linear interactions among semantic, structural, and prestige factors. We do not claim this as a strong theoretical finding; rather, it motivates the use of ensemble methods as a pragmatic choice for predictive tasks where feature interactions are plausible but not independently verified.
...
The comparison between Dimensions and OpenAlex also reveals an important methodological asymmetry. OpenAlex functions as a semantic+structural framework, whereas Dimensions functions primarily as a structural-only baseline. Because the two datasets differ fundamentally in metadata availability, the comparison should not be interpreted as a perfectly controlled dataset comparison. Rather, the two experiments isolate different sources of predictive signal: the Dimensions framework estimates what can be achieved using structural information alone, whereas OpenAlex demonstrates the substantial additional predictive contribution of semantic similarity once abstracts are available.
