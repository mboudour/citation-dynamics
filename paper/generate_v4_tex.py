...
\begin{abstract}
Predicting which scientific papers will cite each other is a fundamental task in bibliometrics, but machine learning models in this domain frequently suffer from temporal leakage---using future network topology or metadata to predict present citations. In this paper, we introduce a strictly leakage-free methodological framework for citation link prediction. We formally define a taxonomy of temporal leakage (citation, topology, semantic, and metadata) and engineer a feature set that strictly respects temporal causality by capping all network metrics prior to the citing paper's publication year. We evaluate this framework on two independent Library and Information Science (LIS) datasets: Dimensions (662,094 pairs) and OpenAlex (478,698 pairs), using temporally and topically constrained negatives. We explicitly decompose evaluation into four modes: (i) pairwise discriminability, (ii) top-$k$ retrieval realism, (iii) full-corpus recommendation, and (iv) candidate-space complexity. Gradient Boosting models achieve ROC-AUC of 0.895 (Dimensions) and 0.976 (OpenAlex), with performance stable across four rolling temporal test windows. We supplement the standard binary classification evaluation with a top-$k$ ranking task to demonstrate retrieval realism, achieving NDCG@10 of 0.856 and 0.937 respectively. Feature ablation reveals that while prestige and structural overlap are strong predictors, semantic similarity dominates when available. We argue that this high predictability reflects the bounded nature of knowledge diffusion within established disciplines, demonstrating that citation behavior is strongly patterned by historical network states and semantic proximity.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Retrieval Realism (Top-$k$)}: The ability to rank the true cited paper against $k-1$ constrained negatives. This bridges the gap between balanced classification and actual search.
    \item \textbf{Recommendation Realism}: The ability to rank the true cited paper against the entire historical corpus $G_{t_A-1}$.
