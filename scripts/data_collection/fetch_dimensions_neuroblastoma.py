"""
fetch_dimensions_neuroblastoma.py
-------------------------
Fetches neuroblastoma journal articles from Dimensions.ai (1975–2024)
using a single keyword search, year by year, and saves two outputs:

  1. Dimensions_neuroblastoma_1975_2024_raw.json.gz   — raw API response (all fields)
  2. Dimensions_neuroblastoma_1975_2024.parquet       — processed: id, year, title,
                                                abstract, reference_ids (list of pub.* strings)

Keyword: "neuroblastoma" (searched in title and abstract)
    ...
    └── computations/
        └── data_collection/
            ...
            └── data_collection_scripts/
                ...
                └── fetch_dimensions_neuroblastoma.py  ← this script
...
# --- Year-by-year fetch ---
all_records = {}   # id → record dict (deduplication across years)
    ...
    try:
        ...
        for rec in df.to_dict(orient="records"):
            ...
            if pid and pid not in all_records:
                # Safely handle abstract — coerce to string, catch any encoding issues
                try:
...
raw_path = DATA_DIR / "Dimensions_neuroblastoma_1975_2024_raw.json.gz"
...
parquet_path = DATA_DIR / "Dimensions_neuroblastoma_1975_2024.parquet"
